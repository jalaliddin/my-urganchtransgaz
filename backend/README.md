...
- **`leave_requests` and `business_trips` are independent tables, not linked.** Module 15 lists "business trip request" as one of four leave-request *types* going through the two-stage approval workflow; Module 16 separately says "create business trip records" with its own richer fields (destination/purpose/order_number/order_file) and never says these records originate from a request. Rather than inventing a request→record conversion step (extra columns, a follow-up "fill in the order details" endpoint), an approved business-trip-type request and an independently HR-created `business_trips` row are treated as two separate ways to reach the same real-world outcome, and both drive the exact same `Employee.status` sync below — each table stays focused on one job.
- **`Employee.status` is the single source of truth attendance already reads** — `MarkAbsentAttendance` (Phase 3) already matches `EmployeeStatus::Vacation/BusinessTrip/SickLeave` verbatim when building yesterday's attendance record, exactly the hook Phase 3's own README called out in anticipation. A new daily command, `leave:sync-employee-status`, recomputes every non-terminated/inactive employee's status fresh each run — from an approved leave request covering today, else a scheduled business trip covering today, else `active` — the same "recompute don't track transitions" approach as `MarkAbsentAttendance` itself, so an edited or cancelled request/trip is never left stale. `ApproveLeaveRequest` also applies this immediately when approval happens after the range has already started, rather than waiting for the next nightly run. `other`-type leave has no matching `EmployeeStatus` case and never touches status — tracked and approved, but attendance-invisible, since the spec gives it no defined meaning beyond "other absence."
- **Two-stage approval is real state** (`pending` → `department_approved` → `approved`, or `rejected` from either stage), not a boolean. **When the employee's own department has no `manager_id` set, the request is created already at `department_approved`** — computed once at submission — since there's no one to perform stage 1.
- **New permissions follow the exact `module.action` convention every prior phase established** (§7 only ever gives permissions "Examples," not an exhaustive list): `leave_requests.view`/`business_trips.view` (granted to every role, mirroring `documents.view`), `leave_requests.review` (department-manager only, matching Module 15's literal "Department Manager"), `leave_requests.approve`/`business_trips.manage` (hr, organization-admin, central — matching "HR / Administrator" and the existing `documents.approve` combo).
- **This is deliberately the loosest-specified phase**: Module 15 opens with "Add **optional** employee request module" and closes with "**keep this module extensible because exact HR workflow may be defined later**," and the spec's acceptance-criteria checklist has no dedicated section for either module (every other module does) — treated as a signal to ship a reasonable, working MVP of the stated shape rather than searching for a more elaborate HR workflow the spec doesn't actually describe.
- **No hard delete** — a `cancelled` status on both tables (an employee can cancel their own still-in-flight request; HR/admin cancels a business trip), matching every prior module's lifecycle philosophy.
...
| GET/POST | `/api/v1/leave-requests[/{id}]` | `index`/`store` always include the caller's own requests, plus the scoped review queue for reviewer/approver roles |
| POST | `/api/v1/leave-requests/{id}/department-approve` \| `/department-reject` | Stage 1 — `leave_requests.review` (department-manager), scoped to their own department |
| POST | `/api/v1/leave-requests/{id}/approve` \| `/reject` | Stage 2 (final) — `leave_requests.approve` (hr/organization-admin/central); `approve` syncs `Employee.status` immediately if the range already covers today |
| POST | `/api/v1/leave-requests/{id}/cancel` | The request's own employee, only while not yet finally decided |
| GET/POST/PUT | `/api/v1/business-trips[/{id}]` | `index`/`store` likewise always include the caller's own trips; `store`/`update`/`cancel` require `business_trips.manage`, scoped to the target employee's organization/department |
| GET | `/api/v1/business-trips/upcoming` | The caller's own scheduled trips ending today or later — powers the dashboard widget |
| POST | `/api/v1/business-trips/{id}/cancel` | No hard-delete endpoint exists |
