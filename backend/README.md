...
| GET/POST | `/api/v1/issues[/{id}]` | `index` scoped to leadership (technical-policy/central-admin/super-admin) sees all, else own department only; `store` requires `issues.create` |
