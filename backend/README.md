...
- **Issues carry an organization, a category, and a responsible employee.** The report form picks the subordinate organization from a list (`issues.organization_id`), a category from `issue_categories` (seeded reference data — gas leak, pipeline damage, low pressure, supply outage, meter fault, station fault, other — so reports can be grouped and counted; new categories are just rows, and `status=inactive` retires one without breaking old issues), and a responsible employee (`issues.responsible_employee_id`). Who chooses the responsible person is decided once, server-side, by `User::mustChooseIssueResponsible()`: Technical Policy Service (and central roles) must name someone in the chosen organization, while a department-manager is *always* the responsible person for what they report — any responsible id they send is ignored, not trusted. `GET /issues/options` and `GET /issues/responsible-candidates` feed the form so the web and mobile clients don't re-derive those rules from roles; candidates are limited to `Employee::scopeIssueHandlers()` (still employed, active account, holds `issues.view`) and the same rule is re-checked in `StoreIssueRequest::after()`, so a crafted request can't name an unrelated organization's employee or someone who couldn't open the issue. The issue's department is taken from the responsible employee, not from the request, and a responsible employee can always view (and is notified of) an issue assigned to them even outside their own department.
...
| GET/POST | `/api/v1/issues[/{id}]` | `index` (filterable by `status`, `issue_category_id`, `organization_id`, `responsible_employee_id`): leadership (technical-policy/central-admin/super-admin) sees all, everyone else their own department plus issues assigned to them; `store` requires `issues.create`, an `issue_category_id`, and — unless the caller is a department-manager, who is responsible for their own report — an `organization_id` and `responsible_employee_id` |
| GET | `/api/v1/issues/options` \| `/responsible-candidates?organization_id=` | Feed the report form: allowed organizations/active categories/`must_choose_responsible`, and the employees who may be named responsible in an organization (`issues.create`; candidates are 403 for a department-manager) |
