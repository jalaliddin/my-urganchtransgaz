...
The employee is always resolved server-side from `employee_number` — a raw `employee_id` is never accepted from the payload. A device that only knows a Dahua terminal's own person id sends `dahua_person_id` instead (resolved against `employees.dahua_person_id`, set once per employee on the Employees page — "Dahua terminal ID"); exactly one of the two fields is accepted per event, never both. See [`../dahua-bridge/README.md`](../dahua-bridge/README.md) for the local bridge that talks to an actual Dahua turnstile and calls this same endpoint.
...
- **`employees.dahua_person_id` is a second, optional way to resolve the employee on that same webhook**, added for a Dahua turnstile bridge (`../dahua-bridge/`) that only ever knows the terminal's own enrollment id for a person, never this app's `employee_number`. It is nullable and unique, set once per employee on the Employees page; `RecordAttendanceEventRequest` requires exactly one of `employee_number`/`dahua_person_id` per event (`required_without`/`prohibits`), so the controller never has to guess which id a given payload meant.
...
- **The report is one endpoint, not separate daily/weekly/monthly ones** — `GET /attendance/report?group_by=employee|department|organization&from=&to=` — with every aggregate (`total_days`, `total_worked_minutes`, one `_count` per `AttendanceStatus` case — `present`, `late`, `early_leave`, `absent`, `business_trip`, `vacation`, `sick_leave`) computed in SQL (`SUM`/`COUNT` with `GROUP BY`), never by loading records into PHP. `GET /attendance/{id}` (or `GET /attendance?filter[employee_id]=&from=&to=`) is the day-by-day detail behind one report row — the report itself deliberately stays an aggregate, not a place to also embed every underlying record.
- **`GET /attendance/timesheet?month=YYYY-MM`** is the monthly "tabel": one row per employee, one column per calendar day, each cell a status + worked minutes (`App\Actions\Attendance\BuildTimesheet`). Unlike `report()`, this is *not* a single aggregate SQL query — a grid with one column per day has no fixed, statically-selectable column list the way a flat sum/count does, and a month's records for the employees in scope are few enough that loading them once and folding the grid in PHP is simpler and no slower in practice than a dynamic pivot query. Non-working days (`Setting::get('attendance.working_days')`, same source `attendance:mark-absentees` already uses) are flagged `is_weekend` rather than left ambiguous with "no record yet". Exports (CSV/XLSX/PDF, via `App\Actions\Export\ExportTimesheet`) print each day as `AttendanceStatus::shortCode()` — `K`/`K/K`/`K/E`/`N`/`X`/`T`/`B`/`D` — the abbreviated form a paper tabel already uses in this region, not something invented for this app.
- **Scoping follows the same convention as Documents (Phase 2)**: `attendance.view` is granted broadly (including the base `employee` role) and, combined with `hasCentralAccess()`/`department-manager` checks, controls the org/department breadth of the index/today/report/timesheet queries — it is not "self vs. everyone," matching the precedent already shipped for documents rather than inventing a stricter model just for this module.
