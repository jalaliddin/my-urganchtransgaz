...
const { items, total, loading, page, itemsPerPage, search, reload } =
  usePaginatedResource(departmentService.list)
...
const headers = computed(() => [
  { title: t('departments.name'), key: 'name' },
  { title: t('departments.code'), key: 'code' },
  { title: t('departments.organization'), key: 'organization.name', sortable: false },
...
const form = ref({ organization_id: null as number | null, name: '', code: '', description: '' })
...
function openCreate() {
  editing.value = null
  form.value = { organization_id: organizations.value[0]?.id ?? null, name: '', code: '', description: '' }
  formErrors.value = {}
...
async function save() {
  ...
  try {
    if (editing.value) {
      await departmentService.update(editing.value.id, form.value)
    } else {
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await departmentService.remove(deleteTarget.value.id)
...
<template>
  <AppPageHeader :title="$t('departments.title')">
    <template #actions>
      <v-btn
        v-if="auth.can('departments.create')"
        color="primary"
  ...
  >
    ...
    <template #item.actions="{ item }">
      <v-btn
        v-if="auth.can('departments.update')"
    ...
    <template #empty>
      <AppEmptyState icon="mdi-sitemap-outline" />
    </template>
  </AppDataTable>
      ...
      <v-card-text>
        <v-form @submit.prevent="save">
          <v-select
            ...
            :label="$t('departments.organization')"
